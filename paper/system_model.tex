...
The type well-formedness judgement is written as \localkinded{\Gamma_e}{t_e}, where the \emph{kinding context} $\Gamma_e$ is a list of in-scope type variables, and $t_e$ is a type of the local language.
The typing judgement is written as \localtyped{\Gamma_e}{\Sigma_e}{e}{t_e}, where $\Gamma_e$ is a local kinding context, the \emph{typing context} $\Sigma_e$ is a list of ascriptions $x \ty t$ of expression variables to types, $e$ is an local expression, and $t_e$ is a local type.
...
\begin{itemize}
	...
	\item (\textsc{Location Name Invertibility}) The type \Loc is invertible for values: if $v$ is a value and $\localemptyped{v}{\Loc}$ then $v \in \Locations$.
