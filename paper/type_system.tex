...
This kind of type consists of precisely the concrete locations $L \in \Locations$, and the location variables $\alpha \knd *_\ell$ that have been bound by the for-all quantifier.
...
Specifically, a program which take as input a type $t$ of kind $\kappa$ (from any of the four kinds mentioned above), and results in a value of type $\subst{\tau}{\alpha}{t}$.

These three types have the following kinding rules:
\begin{mathpar}
  \infer[TyAt]{\localkinded{\proj{\Gamma}{\Locations}}{t_e} \\
...
Types of kind $*_e$ are precisely the types included in the local language.
...
Where the choreographic context $\Delta$ handles variables bound at the choreographic level, for instance via a lambda-abstraction $\lambda X \ty \tau.C$, the local context $\Sigma$ handles variables bound to the local system at a specific location via the local-let binding $\LetIn{\ell.x \ty t_e}{C_1}{C_2}$.
...
Parametric type and location polymorphism is handled in a manner identical to System F.
One can introduce a type variable using the \emph{type abstraction} expression $\Lambda \alpha \knd \kappa.C$, where $\kappa$ can be any of the four kinds of types.
One can then use these type variables in the body of the abstraction, as in the term $\Lambda \alpha \knd *_\ell.\send{C}{L}{\alpha}$ which sends a value $C \ty t_e @ L$ to an arbitrary location $\alpha$.
