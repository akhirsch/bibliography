...
\begin{figure}[h]
  \begin{syntax}
	...
	\alternative{\LetTellIn{\ell}{C_1 \knd *_\ell}{\rho}{C_2}}\\
	\alternative{\LetTellIn{\ell}{C_1 \knd *_e}{\rho}{C_2}}
  \end{syntax}
...
\begin{figure}[h]
  \begin{syntax}
  	...
  	\\
  	\category[Choreography Kinding Context]{\Gamma}
  	\alternative{\cdot}
  	\alternative{\Gamma, \alpha \knd \kappa}
  	
  	\category[Choreography Typing Context]{\Delta}
  	\alternative{\cdot}
  	\alternative{\Delta, X \ty \tau}
  	
  	\category[Locally-Bound Typing Context]{\Sigma}
  	\alternative{\cdot}
  	\alternative{\Sigma, \ell.x \ty t_e}
  	\\
  	\category[Local Kinding Context]{\Gamma_e}
  	\alternative{\cdot}
  	\alternative{\Gamma_e, \alpha}
  	
  	\category[Local Typing Context]{\Delta_e}
  	\alternative{\cdot}
  	\alternative{\Delta_e, x \ty t_e}
  \end{syntax}
