...
\ethan{I cut this discussion down and cleaned it up a bit.
  It also feels like the wrong place, but it needs to be somewhere.}
...
To avoid this namespace capture, location substitution in local let expressions must rename variables when names collide.
In particular, if the location we are substituting \emph{in} binds a variable that is already used by the variable we are substituting \emph{out},
we rename the newly bound variable to avoid any collision.
...
allowing us to define safe location substitution as follows.
\begin{align*}
  \subst{(\LetIn{\alpha.x\ty t}{C_1}{C_2})}{\alpha}{\ell} & = \LetIn*{\ell.x\ty t}{\subst{C_1}{\alpha}{\ell}}{\subst{C_2}{\alpha}{\ell}}
  &&
  \\[1ex]
  \subst{(\LetIn{\ell.x\ty t}{C_1}{C_2})}{\alpha}{\ell} & = \LetIn*{\ell.x\ty t}{\subst{C_1}{\alpha}{\ell}}{\subst{C_2}{\alpha}{\ell}}
  && \text{if}~\alpha.x \notin \fv(C_2)
  \\[1ex]
