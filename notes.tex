...
\section{Syntax}
\begin{syntax}
	\categoryFromSet[Concrete Locations]{L}{\mathcal{L}}
	\category[Synchronization Labels]{d}
	\alternative{L}
	\alternative{R}
	\\
	\category[Local Kinds]{\kappa_e}
	\alternative{*_e}
	\alternative{*_\ell}
	...
	\alternative{X}
	\alternative{t @ \ell}
	\alternative{\kappa_e @ \ell}
	\alternative{\tau_1 \rightarrow \tau_2}
	\alternative{\forall X : \kappa.\tau}
	
	\category[General Types]{T}
	\alternative{t}
	\alternative{\ell}
	\alternative{\tau}
	\\
	\category[Choreographies]{\tau}
	\alternative{x}
	\alternative{\ell.e}
...
\begin{syntax}
	...
	\category[Local Kinding Context]{\Theta}
	\alternative{\cdot}
	\alternative{\Theta, \ell.X : \kappa_e}

	\category[Choreography Typing Context]{\Gamma}
	\alternative{\cdot}
	\alternative{\Gamma, x : \tau}
	
	\category[Local Typing Context]{\Delta}
	\alternative{\cdot}
	\alternative{\Delta, \ell.x : t}
	\\
	\category[Local Expression Kinding Context]{\Theta_e}
	\alternative{\cdot}
	\alternative{\Theta_e, X : \kappa_e}
	
	\category[Local Expression Typing Context]{\Delta_e}
	\alternative{\cdot}
...
\begin{mathparpagebreakable}
  ...
  \infer[AddLocalKind]{\Sigma \vdash \ell : *_\ell \\
  \Sigma \vdash \Theta}
  {\Sigma \vdash (\Theta, \ell.X : \kappa_e)} \and
  
   \infer[EmpTy]{\Sigma \vdash \Theta}
  {\Sigma;\Theta \vdash \cdot} \and

  \infer[AddTy]{\Sigma;\Theta \vdash \tau : * \\
	\Sigma;\Theta \vdash \Gamma}
  {\Sigma;\Theta \vdash (\Gamma, x : \tau)} \and
	
  \infer[EmpLocalTy]{\Sigma \vdash \Theta}
	{\Sigma;\Theta \vdash \cdot} \and
	
  \infer[AddLocalTy]{\Sigma;\Theta \vdash \ell : *_\ell \\
  	\proj{\Sigma}{\ell} + \proj{\Theta}{\ell} \vdash_e t : *_e \\
    \Sigma;\Theta \vdash \Delta}
  {\Sigma;\Theta \vdash (\Delta, \ell.x : t)}
...
\begin{mathparpagebreakable}
	...
	\Sigma \vdash \Theta}
	{\Sigma;\Theta \vdash X : \kappa} \and
	
	\infer[Loc]{\Sigma \vdash \Theta}
	{\Sigma;\Theta \vdash L : *_\ell} \and

	\infer[At]{\Sigma;\Theta \vdash \ell : *_\ell \\
	\proj{\Sigma}{\ell} + \proj{\Theta}{\ell} \vdash_e t : *_e}
	{\Sigma;\Theta \vdash t @ \ell : *}
	
	\infer[TypeAt]{\Sigma;\Theta\vdash \ell : *_\ell}
	{\Sigma;\Theta \vdash \kappa_e @ \ell : *}
	
	\infer[Arrow]{\Sigma;\Theta \vdash \tau_1 : *\\
	  \Sigma;\Theta \vdash \tau_2 : *}
	{\Sigma;\Theta \vdash \tau_1 \rightarrow \tau_2 : *}
	
	\infer[ForAll]{(\Sigma, X : \kappa);\Theta \vdash \tau : *}
	{\Sigma;\Theta \vdash \forall X : \kappa.\tau : *}
\end{mathparpagebreakable}

\paragraph{Choreography Typing Rules}\;
\begin{mathparpagebreakable}
  \infer[Var]{\Sigma \vdash \Theta \\
  	\Sigma;\Theta \vdash \Gamma \\
  	\Sigma;\Theta \vdash \Delta \\
    x : \tau \in \Gamma}
  {\Sigma;\Theta;\Gamma;\Delta \vdash x : \tau} \and

  \infer[Local]{\Sigma \vdash \Theta \\
  	\Sigma;\Theta \vdash \Gamma \\
  	\Sigma;\Theta \vdash \Delta \\
  	\Sigma;\Theta \vdash \ell : *_\ell\\
  	(\proj{\Sigma}{\ell} + \proj{\Theta}{\ell}; \proj{\Delta}{\ell}) \vdash_e e : t}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \ell.e \vdash t @ \ell} \and

  \infer[Send]{\Sigma;\Theta;\Gamma;\Delta \vdash C : t @ \ell_1 \\
	\proj{\Sigma}{\ell_1} + \proj{\Theta}{\ell_1} \vdash_e \ell_2 : *_\ell}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \send{C}{\ell_1}{\ell_2} : t @ \ell_2} \and

  \infer[SendTy]{\Sigma;\Theta;\Gamma;\Delta \vdash C : \kappa_e @ \ell_1 \\
	\proj{\Sigma}{\ell_1} + \proj{\Theta}{\ell_1} \vdash_e \ell_2 : *_\ell}
	{\Sigma;\Theta;\Gamma;\Delta \vdash \send{C}{\ell_1}{\ell_2} : \kappa_e @ \ell_2} \and

  \infer[Sync]{\Sigma \vdash \ell_1 : *_\ell \\
    \proj{\Sigma}{\ell_1} + \proj{\Theta}{\ell_1} \vdash_e \ell_2 : *_\ell\\
    \Sigma;\Theta;\Gamma;\Delta \vdash C : \tau}
	{\Sigma;\Theta;\Gamma;\Delta \vdash \send{\ell_1[d]}{}{\ell_2}; C : \tau} \and

  \infer[If]{\Sigma;\Theta;\Gamma;\Delta \vdash C_1 : \text{bool} @ \ell \\
    \Sigma;\Theta;\Gamma;\Delta \vdash C_1 : \tau \\
    \Sigma;\Theta;\Gamma;\Delta \vdash C_2 : \tau}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \ell.\ifthen{C}{C_1}{C_2}} \and

  \infer[Fun]{\Sigma;\Theta;(\Gamma, x : \tau_1);\Delta \vdash C : \tau_2}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \lambda x : \tau_1.C : \tau_1 \rightarrow \tau_2} \and

  \infer[Rec]{\Sigma;\Theta;(\Gamma, x : \tau);\Delta \vdash C : \tau}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \mu x : \tau.C : \tau} \and

  \infer[App]{\Sigma;\Theta;\Gamma;\Delta \vdash C_1 : \tau_1 \rightarrow \tau_2 \\
    \Sigma;\Theta;\Gamma;\Delta \vdash C_2 : \tau_1}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \app{C_1}{C_2} : \tau_2} \and

  \infer[AbsTy]{(\Sigma, X : \kappa);\Theta;\Gamma;\Delta \vdash C : \tau}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \Lambda X : \kappa.C : \forall X : \kappa.\tau} \and

  \infer[AppTy]{\Sigma;\Theta;\Gamma;\Delta \vdash C : \forall X : \kappa.\tau \\
    \Sigma;\Theta \vdash T : \kappa}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \app{C}{T} : \tau[X \mapsto T]} \and

  \infer[LocalLet]{\Sigma;\Theta;\Gamma;\Delta \vdash C_1 : t_1 @ \ell \\
	\Sigma;\Theta;\Gamma;(\Delta, x : \ell.t_1) \vdash C_2 : \tau_2}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \letin{\ell.x : t_1}{C_1}{C_2}} \and

  \infer[LocalLetTy]{\Sigma;\Theta;\Gamma;\Delta \vdash C_1 : \kappa_e @ \ell \\
  	\Sigma;\Theta;\Gamma;\Delta \vdash \tau : *\\
    \Sigma;(\Theta, \ell.X : \kappa_e);\Gamma;\Delta \vdash C_2 : \tau}
  {\Sigma;\Theta;\Gamma;\Delta \vdash \letin{\ell.X : \kappa_e}{C_1}{C_2} : \tau}
\end{mathparpagebreakable}
