...
\begin{syntax}
	...
	\alternative{L}
	\alternative{L_1 \rightsquigarrow L_2}\\
	\alternative{L_1[d] \rightsquigarrow L_2}
	\alternative{L_1 \rightsquigarrow \rho}

	\category[Traces]{\sigma}
	\alternative{\epsilon}
	\alternative{\sigma, T}
\end{syntax}

\paragraph{Trace Semantics}\;
Deterministic, small-step trace semantics $C_1 \step{\sigma} C_2$ where $C_1, C_2$ are \emph{closed} choreographies, and $\sigma$ is a trace.

\begin{mathparpagebreakable}
  \infer[Done]{e_1 \Rightarrow_e e_2}
  {L.e_1 \step{\epsilon, L} L.e_2} \and

  \infer[Send]{C_1 \step{\sigma} C_2}
  {C_1 \overset{L_1}{\rightsquigarrow} L_2 \step{\sigma} C_2 \overset{L_1}{\rightsquigarrow} L_2} \and

  \infer[SendV]{\text{Value}(v)}
  {L_1.v \overset{L_1}{\rightsquigarrow} L_2 \step{\epsilon, L_1 \rightsquigarrow L_2} L_2.v} \and

  \infer[If]{C \step{\sigma} C'}
  {L.\text{if}\; C \;\text{then}\; C_1 \;\text{else}\; C_2 \step{\sigma} L.\text{if}\; C' \;\text{then}\; C_1 \;\text{else}\; C_2} \and

  \infer[IfT]{}
  {L.\text{if}\; L.\text{true} \;\text{then}\; C_1 \;\text{else}\; C_2 \step{\epsilon, L} C_1} \and

  \infer[IfF]{}
  {L.\text{if}\; L.\text{false} \;\text{then}\; C_1 \;\text{else}\; C_2 \step{\epsilon, L} C_2} \and

  \infer[DefLocal]{C_1 \step{\sigma} C_1'}
  {\text{let}\; L.x := C_1 \;\text{in}\; C_2 \step{\sigma} \text{let}\; L.x := C_1' \;\text{in}\; C_2} \and

  \infer[DefLocalV]{\text{Value}(v)}
  {\text{let}\; L.x := L.v \;\text{in}\; C \step{\epsilon} C[x \mapsto v]} \and

  \infer[AppFun]{C_1 \step{\sigma} C_1'}
  {C_1 \; C_2 \step{\sigma} C_1' \; C_2} \and

  \infer[AppArg]{C_2 \step{\sigma} C_2'}
  {C_1 \; C_2 \step{\sigma} C_1 \; C_2'} \and

  \infer[App]{\text{Value}(V)}
  {(\text{fun}\; \phi(\alpha) := C) \; V \step{\sigma} C[\phi \mapsto \text{fun}\; \phi(\alpha) := C, \alpha \mapsto V]} \and

  \infer[Sync]{}
  {L_1[d] \rightsquigarrow L_2; C \step{\epsilon, L_1[d] \rightsquigarrow L_2} C} \and

  \infer[LocApp]{}
  {(\Lambda X.C) \; L \step{\epsilon} C[X \mapsto L]} \and

  \infer[TellLet]{C_1 \step{\sigma} C_1'}
  {L.\text{tell}\; \rho_1\; \text{let}\; X := C_1 \in \rho_2 \;\text{in}\; C_2 \step{\sigma} L.\text{tell}\; \rho_1\; \text{let}\; X := C_1' \in \rho_2 \;\text{in}\; C_2}

  \infer[TellLetV]{}
  {L_1.\text{tell}\; \rho_1\; \text{let}\; X := L_1.L_2 \in \rho_2 \;\text{in}\; C \step{\epsilon, L_1 \rightsquigarrow \rho_1} C[X \mapsto L_2]}
...
\begin{aligned}[t]
	...
	\epp{\ell_1.e \rightsquigarrow \alpha @ \ell_2; C}{L} &=
		\begin{cases}
			\text{let}\; \alpha := \text{ret}(e) \;\text{in}\; \epp{C}{L} & L = \ell_1 = \ell_2 \\
			\text{send}\; e \;\text{to}\; \ell_2; \epp{C}{L} & L = \ell_1 \neq \ell_2 \\
			\text{receive}\; \alpha \;\text{from}\;\ell_1 ; \epp{C}{L} & L = \ell_2 \neq \ell_1\\
			\epp{C}{L} & L \neq \ell_1, \ell_2
		\end{cases}\\
	\epp{\text{if}\; \ell.e \;\text{then}\; C_1 \;\text{else}\; C_2}{L} &= 
		\begin{cases}
			\text{if}\; e \;\text{then}\; \epp{C_1}{L} \;\text{else}\; \epp{C_2}{L} & L = \ell \\
	...
	\epp{\text{let}\; \ell.x := C_1 \;\text{in}\; C_2}{L} &= 
		\begin{cases}
			...
			(\text{fun}\; \phi(\alpha) := \epp{C_2}{L})\;\epp{C_1}{L} & L \neq \ell
		\end{cases}\\
	\epp{\text{fun}\; \phi(\alpha) := C}{L} &= \text{fun}\; \phi(\alpha) := \epp{C}{L}\\
	\epp{C_1\;C_2}{L} &= \epp{C_1}{L}\;\epp{C_2}{L}\\
	\epp{\Lambda X.C}{L} &= \Lambda X.\text{AmI}\; X \;\text{?}\; \epp{C[X \mapsto L]}{L} \;\text{:}\; \epp{C}{L}\\
	\epp{C\;\ell}{L} &= \epp{C}{L}\;\ell\\
	\epp{\text{tell}\; X := \ell.e \;\text{do}\; C}{L} &= 
		\begin{cases}
			\text{send loc}\; e \;\text{to}\; \text{IP}(C); \epp{C}{L} & L = \ell \\
			\text{receive loc}\; X \;\text{from}\; L;\epp{C}{L} & L \in \text{IP}(C), L \neq \ell\\
			\epp{C}{L} & \text{otherwise}
		\end{cases}
\end{aligned}
\end{mathparpagebreakable}
